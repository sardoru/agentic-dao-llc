      ...
      <section className="mb-8">
        ...
        <div className="bg-surface-2 border border-border rounded-lg p-4 space-y-2">
          <div className="flex items-center justify-between text-xs text-muted">
            <span>
              ${treasury.weeklySpentUsd.toLocaleString()} spent this epoch
            </span>
            <span>
              ${treasury.weeklyCapUsd.toLocaleString()} weekly cap
            </span>
          </div>
          ...
          <div className="text-xs text-muted">
            {spendPct}% of weekly cap consumed
          </div>
        </div>
      </section>
              ...
              {treasury.recentSpends.map((spend) => (
                <tr key={spend.id} className="hover:bg-surface-3 transition-colors">
                  <td className="px-4 py-2.5 text-muted">
                    {formatTimestamp(spend.executedAt)}
                  </td>
                  <td className="px-4 py-2.5 font-mono text-muted">
                    {formatAddress(spend.to)}
                  </td>
                  <td className="px-4 py-2.5 font-mono text-ink">
                    {formatBigInt(spend.amount, spend.symbol === "USDC" ? 6 : 18)}{" "}
                    {spend.symbol}
