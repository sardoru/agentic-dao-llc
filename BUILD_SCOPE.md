...
and what actually exists in the repository. The README explains _how the system works_; this
file states _what was built, what is deliberately excluded, and what is left to do._
...
and to _prove_ the alignment holds.
...
docs/             interfaces · runbook (Step 0 = one-command local repro) · threat-model · adr/0001–0005
scripts/          gen-reserved-matters.mjs (codegen + --check drift gate) ·
                  e2e-local.mjs (pnpm e2e:local — scripted end-to-end agent on anvil, 52 assertions)
```

**Test evidence:** 135 unit/contract tests passing — **106 TypeScript** (policy 32, signer 13,
sim 11, mcp 17, cli 10, chain 23) + **29 Foundry** (functional/unit + 11 adversarial safety
proofs) — **plus the `pnpm e2e:local` integration harness (52 assertions)** driving the real
runtime through the full lifecycle against a live anvil. Typecheck, build, and `prettier --check`
are clean across all eight `@agentic-dao/*` packages; `pnpm lint` is green.
...
- **Runtime ↔ legal**: both are _generated_ from the yaml; `check:reserved` re-generates and
...
- **v1 (this build):** all six phases code-complete; 135 tests green; reserved-matters three-way
  invariant holds in CI; the adversarial safety proofs pass; **`pnpm e2e:local` drives the real
  runtime through the full lifecycle on a live anvil (52 assertions).** ✅
- **Testnet live:** issues [#1](https://github.com/sardoru/agentic-dao-llc/issues/1)–[#4](https://github.com/sardoru/agentic-dao-llc/issues/4) + [#16](https://github.com/sardoru/agentic-dao-llc/issues/16) closed — deployed to Base Sepolia, indexer/dashboard reading live events, end-to-end scripted agent run reproduced from the runbook. (The scripted agent itself already runs locally via `pnpm e2e:local`; what remains is a funded testnet deploy + Tenderly/IPFS credentials.)
